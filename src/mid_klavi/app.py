...
class MidKlavi(Handler):
    ...
    def handler(self):
        is_save_handle = False
        is_generate_handle = True
        is_only_logger = False

        if self.body['action'] == 'save':
            is_save_handle = True
            is_generate_handle = False
            is_only_logger = False


        if is_only_logger:
            self.log_request()
            return Result(HTTPStatus.OK, {"message": "Event Logged"})

        if is_save_handle:
            report = self.save_payload_into_database()
            return Result(HTTPStatus.OK, {"id": str(report.id)})

        if is_generate_handle:
            report_id = self.body['report_id']
            enquiry_cpf = self.body['enquiry_cpf']
            report = self.get_report_from_database(report_id, enquiry_cpf)
            xls_stream = self.generate_xlsx_stream_from_report(report)
            self.upload_excel_stream_to_s3(xls_stream, report_id)
            xls_stream.close()
